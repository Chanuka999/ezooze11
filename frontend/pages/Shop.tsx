  ...
  const categoryStructure = useMemo(() => {
    ...
    navLinks.forEach((link) => {
      if (link.filters?.category && link.children) {
        const categoryKey = link.filters.category as
          | "men"
          | "women"
          | "unisex"
          | "sportswear";
    ...
    <div className="py-6 border-b border-gray-200 dark:border-gray-700">
      <h3 className="-my-3 flow-root">
        <button
          ...
          className="flex w-full items-center justify-between bg-white dark:bg-brand-charcoal py-3 text-sm text-gray-400 hover:text-gray-500 dark:text-gray-400 dark:hover:text-gray-200"
        >
          ...
          <span className="ml-6 flex items-center">
            <ChevronDownIcon
              className={`h-5 w-5 transform transition-transform duration-200 ${openSections[sectionKey] ? "rotate-180" : ""}`}
            />
          </span>
        </button>
      </h3>
      <div
        className={`pt-6 overflow-hidden transition-all duration-300 ease-in-out ${openSections[sectionKey] ? "max-h-[1000px]" : "max-h-0"}`}
      >
    ...
    <form className="p-4 lg:p-0">
      <div className="flex justify-between items-center pb-4 border-b border-gray-200 dark:border-gray-700">
        ...
        <button
          ...
          className="text-sm font-medium text-brand-gold hover:text-yellow-600"
        >
          Clear all
        </button>
      </div>
      ...
      <FilterSection title="Category" sectionKey="category">
        <ul className="space-y-2">
          {Object.entries(categoryStructure).map(
            ([cat, subCats]: [string, { name: string; slug: string }[]]) => (
              <li key={cat}>
                <button
                  type="button"
                  onClick={() => handleCategorySelect(cat)}
                  className={`font-medium capitalize ${filters.category === cat && !filters.subCategory ? "text-brand-gold" : "hover:text-brand-gold"}`}
                >
                  {cat}
                </button>
          ...
          {Object.entries(facets.sizes)
            ...
            .map(([size, count]) => (
              ...
              >
                <span>{size}</span>
                <span
                  className={`text-xs ${filters.sizes.includes(size) ? "text-gray-300 dark:text-gray-500" : "text-gray-400"}`}
                >
                  ({count})
                </span>
              </button>
      ...
      <FilterSection title="Materials" sectionKey="materials">
        <div className="flex flex-col gap-2">
          {Object.entries(facets.materials)
            .sort()
            .map(([material, count]) => (
              <button
                type="button"
              ...
              >
                <span>{material}</span>
                <span
                  className={`text-xs ${filters.materials.includes(material) ? "text-gray-300 dark:text-gray-500" : "text-gray-400"}`}
                >
                  {count}
                </span>
              </button>
...
const FilterPill: React.FC<{ label: string; onRemove: () => void }> = ({
  label,
  onRemove,
}) => (
  <span className="flex items-center bg-brand-light-gray dark:bg-gray-700 rounded-full px-3 py-1 text-sm font-medium text-brand-charcoal dark:text-brand-cream transition-all duration-200 animate-fadeIn">
    ...
    <button
      onClick={onRemove}
      className="ml-2 -mr-1 p-0.5 rounded-full hover:bg-gray-300 dark:hover:bg-gray-600 focus:outline-none"
    >
...
export const Shop: React.FC<ShopProps> = ({
  ...
  }, [filters, sortBy, searchQuery, showInStockOnly]);
  ...
  const baseProducts = useMemo(() => {
    return products.filter((p) => {
      ...
      if (filters.subCategory) {
        ...
        if (pSub !== filterSub && pSub.replace(/ /g, "-") !== filterSub)
          return false;
      }
      return true;
    });
  ...
  const filteredProducts = useMemo(() => {
    let filtered = baseProducts.filter((p) => {
      ...
      if (
        ...
        return false;
      if (showInStockOnly && getProductTotalStock(p) === 0) return false;
      if (searchQuery.trim()) {
    ...
    return sorted;
  }, [baseProducts, filters, sortBy, searchQuery, showInStockOnly]);
  ...
  const activeFilterCount = useMemo(() => {
    ...
    return count;
  }, [filters, facets.maxPrice, searchQuery, showInStockOnly]);

  const collectionStats = useMemo(() => {
    const inStock = filteredProducts.filter(
      (p) => getProductTotalStock(p) > 0,
    ).length;
    const onSale = filteredProducts.filter(
    ...
    return { inStock, onSale, lowStock };
  }, [filteredProducts]);

  const removeFilter = (
    key: keyof Omit<ShopFilters, "subCategory">,
    value?: any,
  ) => {
    setFilters((prev) => {
  ...
  const getPageTitle = () => {
    if (filters.subCategory && filters.category) {
      ...
      const subCategoryName = filters.subCategory
        .replace(/-/g, " ")
        .split(" ")
  ...
  const getCategoryFilterLabel = () => {
    if (filters.subCategory && filters.category) {
      const navLink = navLinks.find(
        (l) => l.filters?.category === filters.category,
      );
      const subCat = navLink?.children?.find(
        (c) => c.filters.subCategory === filters.subCategory,
      );
  ...
  return (
    <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 animate-fadeIn">
      ...
      <div
        className={`fixed inset-0 bg-black bg-opacity-25 z-[60] transition-opacity lg:hidden ${isFilterDrawerOpen ? "opacity-100" : "opacity-0 pointer-events-none"}`}
        onClick={() => setIsFilterDrawerOpen(false)}
        aria-hidden="true"
      />
      <div
        className={`fixed inset-y-0 left-0 w-full max-w-sm bg-brand-cream dark:bg-brand-charcoal z-[70] transform transition-transform ease-in-out duration-300 lg:hidden ${isFilterDrawerOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
      ...
      <div className="pt-12">
        <h1 className="text-center text-4xl font-serif font-bold tracking-tight text-brand-charcoal dark:text-brand-cream">
          {getPageTitle()}
        </h1>

        <div className="mt-8 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-brand-surface/70">
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            <div className="relative lg:col-span-2">
              ...
              <input
                ...
                placeholder="Search by product name, material, color, or category"
                className="w-full rounded-xl border border-gray-200 bg-gray-50 py-3 pl-10 pr-10 text-sm text-gray-900 focus:border-brand-gold focus:outline-none focus:ring-2 focus:ring-brand-gold/30 dark:border-gray-600 dark:bg-brand-charcoal dark:text-brand-cream"
              />
              {searchQuery && (
                <button
                  type="button"
                  onClick={() => setSearchQuery("")}
                  className="absolute right-2 top-1/2 -translate-y-1/2 rounded-md p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-700 dark:hover:bg-gray-700 dark:hover:text-gray-200"
            ...
            <button
              ...
              className={`rounded-xl border px-4 py-3 text-sm font-medium transition-colors ${showInStockOnly ? "border-emerald-500 bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300" : "border-gray-200 bg-gray-50 text-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:bg-brand-charcoal dark:text-gray-300 dark:hover:bg-gray-700"}`}
            >
              {showInStockOnly ? "In-stock only: On" : "Show in-stock only"}
            </button>
          </div>

          <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
            <div className="rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm dark:border-gray-700 dark:bg-brand-charcoal">
              <p className="text-xs uppercase tracking-wide text-gray-500">
                Results
              </p>
              <p className="mt-1 text-lg font-semibold text-brand-charcoal dark:text-brand-cream">
          ...
          <div className="lg:col-span-3">
            <div className="flex items-center justify-between pb-6 border-b border-gray-200 dark:border-gray-700">
              <div className="flex items-center space-x-4">
                ...
                <button
                  ...
                  className="lg:hidden relative flex items-center text-sm font-medium p-2 px-4 border rounded-md dark:border-gray-600"
                >
            ...
            {activeFilterCount > 0 && (
              <div className="py-4 flex flex-wrap gap-2 items-center border-b border-gray-200 dark:border-gray-700">
                {filters.category && (
                  <FilterPill
                    label={`Category: ${getCategoryFilterLabel()}`}
                ...
                {filters.maxPrice &&
                  ...
                  filters.maxPrice < facets.maxPrice && (
                    <FilterPill
                      label={`Price: < ${formatPrice(filters.maxPrice, storeSettings.currency)}`}
                ...
                {filters.colors.map((c) => (
                  <FilterPill
                    key={c}
                    label={c}
                    onRemove={() => removeFilter("colors", c)}
                  />
                ))}
                {filters.sizes.map((s) => (
                  <FilterPill
                ...
                {filters.materials.map((m) => (
                  <FilterPill
                    key={m}
                    label={m}
                    onRemove={() => removeFilter("materials", m)}
                  />
                ))}
                <button
                  onClick={clearFilters}
                  className="text-sm font-medium text-brand-gold hover:text-yellow-600 hover:underline ml-2"
                >
                  Clear all
                </button>
              </div>
