...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const appleClientId = (import.meta as any)?.env?.VITE_APPLE_CLIENT_ID as string | undefined;
  const appleRedirectUri = (import.meta as any)?.env?.VITE_APPLE_REDIRECT_URI || window.location.origin;
  ...
  const signInWithApple = async (): Promise<User> => {
    try {
      clearError();
      if (!appleClientId) {
        throw new Error('Apple Sign-In is not configured. Set VITE_APPLE_CLIENT_ID.');
      }

      const { idToken, email, name } = await getAppleIdToken(appleClientId, appleRedirectUri);
